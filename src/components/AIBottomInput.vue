...
async function startRecording() {
  try {
    ...
    ElMessage.error(`无法开始录音：${error.message || '请检查麦克风权限'}`)
  }
}
...
async function transcribeAudio() {
  ...
  try {
    ...
    ElMessage.error(`语音识别失败：${error.message || '请稍后再试'}`)
  } finally {
    ...
    <div class="composer-panel">
      ...
      <div class="composer-row">
        <textarea
          ...
          v-model="inputValue"
          rows="2"
          :placeholder="isTranscribing ? '正在识别语音...' : '描述设备现象、故障代码、检修步骤，AI 会结合知识库给出建议'"
